...
def generate_mask(img: np.ndarray, blk_list: List[TextBlock], default_padding: int = 5) -> np.ndarray:
    ...
    for blk in blk_list:
        # Ensure inpaint_bboxes is a list
        bboxes = blk.inpaint_bboxes if isinstance(blk.inpaint_bboxes, list) else []
        
        if not bboxes:  # If no inpaint boxes, use the main text box
            bboxes = [(blk.xyxy[0], blk.xyxy[1], blk.xyxy[2], blk.xyxy[3])]
...
def set_alignment(blk_list):
    text_render_settings = "Center"
    for blk in blk_list:
        alignment = text_render_settings
        if alignment == "Center":
